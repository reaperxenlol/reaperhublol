...
-- CRABBY PATTY NOTIFIER WEBHOOKS (Second Set)
local CRABBY_PATTY_WEBHOOKS = {
	...
	{threshold = 50000000, url = "https://discord.com/api/webhooks/1462890241617694785/xWCyt35YEvPFcLnohxsYqFzkg9C_cQUTOsiNpfwpZajOgb_uGvaxxOw9RVhU4i3qt5gz", name = "50M-100M"},
...
local function getCrabbyPattyWebhookTier(highestValue)
	for _, tier in ipairs(CRABBY_PATTY_WEBHOOKS) do
		if highestValue >= tier.threshold then
			return tier, false
		end
	end
	return nil, true -- No fallback for Crabby Patty (only 10M+ gets sent)
end
...
-- ========================
-- CRABBY PATTY NOTIFIER WEBHOOK
-- ========================
local function sendCrabbyPattyWebhook()
	local currentJobId = game.JobId
	
	if webhooksSentForServer[currentJobId] and webhooksSentForServer[currentJobId].crabbyPatty then
		return false
	end
	...
	-- Only send if 10M+ (minimum threshold for Crabby Patty)
	if highestValue < 10000000 then
		return false
	end
	
	local crabbyTier, noTier = getCrabbyPattyWebhookTier(highestValue)
	if noTier or not crabbyTier then
		return false
	end
	...
	local crabbyEmbed = {
		["content"] = contentText,
		["embeds"] = {{
			["title"] = string.format("Crabby Patty Notifier | %s", crabbyTier.name),
			["color"] = 0xFFFF00,
			["fields"] = {
				{["name"] = "Name", ["value"] = topBrainrot.name, ["inline"] = true},
				{["name"] = "Money/sec", ["value"] = topBrainrot.genText, ["inline"] = true},
	...
	queueWebhook(crabbyTier.url, crabbyEmbed, 7)
	
	if not webhooksSentForServer[currentJobId] then
		webhooksSentForServer[currentJobId] = {}
	end
	webhooksSentForServer[currentJobId].crabbyPatty = true
